...
func NewService(cfg *core.Config, sessionStore sessions.Store) *Service {
	return &Service{
...
func (s *Service) GetSession() (*sessions.Session, error) {
	// Make sure StartSession has been called
	if s.session == nil {
		return nil, ErrSessonNotStarted
	}

	// Retrieve our session store struct and type-assert it

...
func (s *Service) FindUserByUsername(username string) (*models.OauthUser, error) {
	...
	notFound := s.db.Where("username = ?", username).
...
// FindUserByID looks up a user by userID
func (s *Service) FindUserByID(userID string) (*models.OauthUser, error) {
	user := new(models.OauthUser)
	notFound := s.db.Where("id = ?", userID).
		First(user).RecordNotFound()
...
// AuthUserByID authenticates user
func (s *Service) AuthUserByID(userID string) (*models.OauthUser, error) {
	// Fetch the user

...
func (suite *OauthTestSuite) TestAuthenticateRollingRefreshToken() {
	...
	assert.Equal(
		suite.T(),
		now1.Unix()+int64(suite.cfg.Oauth.RefreshTokenLifetime),
	...
	assert.Equal(
		suite.T(),
		now2.Unix()+int64(suite.cfg.Oauth.RefreshTokenLifetime),
		refreshTokens[1].ExpiresAt.Unix(),
	)
	assert.Equal(suite.T(), "test_token_3", refreshTokens[2].Token)
	assert.Equal(
	...
	assert.Equal(
		suite.T(),
		now3.Unix()+int64(suite.cfg.Oauth.RefreshTokenLifetime),
		refreshTokens[2].ExpiresAt.Unix(),
	)
}
